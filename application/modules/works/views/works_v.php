						...
						<div class="">
							<div class="box-head pad-bottom-10 clearfix">
								...
								<label><?php echo $screen ?></label>
          						...
          						<span class="fa fa-film pointer play_works_vids open_help_vids_mpd" data-toggle="modal" data-target="#help_video_group"> </span>
          						<button type = "button" class = "btn btn-success btn-sm" v-on:click = "viewContractorCertificate">View Covid 19 Certificates</button>
          						<button type = "button" class = "btn btn-primary btn-sm" v-on:click = "viewSiteLogin">View Site Login</button>
           ...
           <!--  <div class="col-sm-12 pad-5">
            ...
            <div class="col-sm-12 pad_5" style = "height: 400px; overflow: auto">
              <table class = "table  table-condensed table-bordered m-bottom-0 table-striped fancyTable">
                ...
                <tr><td v-on:click = "select_contractor_company(0)">Focus Shopfit</td></tr>
          ...
          <div class="col-sm-8">
            ...
            <div class="col-sm-12 pad-5" style ="height: 400px; overflow: auto">
              <table class = "table  table-condensed table-bordered m-bottom-0 table-striped fancyTable">
                ...
                <tr v-for = "ss_covid_cert in ss_covid_cert" v-if = "ss_covid_cert.company_id == cert_selected_comp_id" >
                  <td :class = "checkwatched(ss_covid_cert.date_watched)" v-if = "ss_covid_cert.company_id == 0">
                    {{ ss_covid_cert.user_first_name +" "+ ss_covid_cert.user_last_name}}
                  </td>
                  <td :class = "checkwatched(ss_covid_cert.date_watched)" v-if = "ss_covid_cert.company_id > 0">
...
<div class="modal fade" id="mdl_site_login" role="dialog">
    <div class="modal-dialog" style = "width: 1000px">
        <div class="modal-content">
          
          	<div class="modal-header">
              	<button type="button" class="close" data-dismiss="modal">&times;</button>
              	<h4 class="modal-title">Project Site Login</h4>
          	</div>
          	<div class="modal-body row">
          		 <div class="col-sm-4">
		            <div class="col-sm-12 pad-5"><input type="text" class = "form-control input-sm" v-model = "searchContractorCovid" placeholder = "search contractor..."></div>
		            <div class="col-sm-12 pad_5" style = "height: 400px; overflow: auto">
			            <table class = "table  table-condensed table-bordered m-bottom-0 table-striped fancyTable">
			                ...
			                <tr><td v-on:click = "select_sl_contractor_company('All')">All</td></tr>
			                <tr><td v-on:click = "select_sl_contractor_company(0)">Focus Shopfit</td></tr>
			                <tr v-for = "project_contractors in project_contractors" v-on:click = "select_sl_contractor_company(project_contractors.company_id)">
			                  <td >
			                    {{ project_contractors.works_id +"-"+ project_contractors.company_name }}
			                  </td>
			                </tr>
           		...
           		<div class="col-sm-8 pad-5" style ="height: 400px; overflow: auto">
	              	<table class = "table  table-condensed table-bordered m-bottom-0 table-striped fancyTable">
		                <tr>
		                	<th>Company Name</th>
		                  	<th>Site Staff Name</th>
		                ...
		                <tr v-for = "ss_site_login in ss_site_login">
		                	<td>
		                    	{{ ss_site_login.company_name }}
		                  	</td>
						  	<td>
		                    	{{ ss_site_login.site_staff_name }}
		                  	</td>
		                 	<td>
		                    	{{ ss_site_login.log_date }}
		                  	</td>
		                  	<td>{{ ss_site_login.time_in }}</td>
		                  	<td>{{ ss_site_login.time_out }}</td>
		                </tr>
	              	</table>
	            </div>
          	</div>
		...
		mounted: function(){
  		...
  		methods: {
		    ...
		    fetch_site_staff_covid_cert(){
		        axios.post(baseurl+"induction_health_safety/fetch_company_site_staff_covid_cert_file", 
		        {
		        	'project_id': this.project_id,
		        	'company_id': this.cert_selected_comp_id
		        }).then(response => { 
		        	this.ss_covid_cert = response.data;   
		        	if(this.ss_covid_cert.length == 0){
		        		alert("Site Staff have no Covid Certificates");
		        	}         
		       	}).catch(error => {
		        	console.log(error.response)
		        });
		    ...
		    viewContractorCertificate: function(){
		    	this.fetch_project_contractors();
		    ...
		    view_certificate: function(img_path,company_id){
		        var img_path = baseurl+"uploads/company/covid_cert/"+company_id+"/"+img_path;
		        $("#iframe").attr("src", img_path);
		        $("#mdl_covid_file_preview").modal('show');
	        },

	        printSiteStaffwihCovidCert: function(){
	          	axios.post(baseurl+"induction_health_safety/print_ss_with_covid_cert", 
		        {
		            'project_id': this.project_id
		        }).then(response => { 
		            window.open(baseurl+'docs/site_staff_with_covid_certificates/'+response.data+'.pdf?q='+$.now(), '', 'height=590,width=850,top=100,left=100,location=no,toolbar=no,resizable=yes,menubar=no,scrollbars=yes',true);
		            // this.ss_covid_cert = response.data;          
		        }).catch(error => {
		            console.log(error.response)
		        });
	        ...
	        load_project_site_login: function(){
	        	axios.post(baseurl+"site_login/fetch_project_site_staff", 
		        {
		        	'projects_id': this.project_id
		        }).then(response => { 
		        	this.ss_site_login = response.data;          
		       	}).catch(error => {
		        	console.log(error.response)
		        });
	        },
	        ...
	        viewSiteLogin: function(){
	        	this.fetch_project_contractors();
	        	this.load_project_site_login();
	        	$("#mdl_site_login").modal('show');
	        },

	        select_sl_contractor_company: function(){
