...
{ 
    ...
    $this->Cell(60,10,'Project Details',0,0,'R');
    $this->Ln();
    $this->SetFont('Arial','',12);
    $this->Cell(5);
...
$pdf->Cell(115);
$pdf->Cell(25,10,"Price",0,0,'R');
$pdf->Cell(25,10,"Estimate",0,0,'R');
$pdf->Cell(25,10,"Quoted",0,0,'R');
$pdf->Ln(8);
$t_price = 0;
$t_estimate = 0;
$t_quoted = 0;
foreach ($works_t->result_array() as $works_row){
  ...
  $pdf->Cell(5,10,'',0,0);
  $pdf->SetFont('Arial','',9);
  $pdf->Cell(55,10,$work_description,0,0);
  $pdf->SetFont('Arial','',9);
  $pdf->Cell(55,10,$works_row['company_name'],0,0);
  $pdf->SetFont('Arial','',9);
  $pdf->Cell(25,10,number_format($works_row['price']),0,0,'R');
  $pdf->Cell(25,10,number_format($works_row['work_estimate']),0,0,'R');
  $pdf->Cell(25,10,number_format($works_row['total_work_quote']),0,0,'R');
  $pdf->Ln(5);

  $t_price = $t_price + $works_row['price'];
  $t_estimate = $t_estimate + $works_row['work_estimate'];
  $t_quoted = $t_quoted + $works_row['total_work_quote'];
}
$pdf->Ln(5);
...
$pdf->SetFont('Arial','B',9);
$pdf->Cell(115);
$pdf->Cell(25,10,number_format($t_price),0,0,'R');
$pdf->Cell(25,10,number_format($t_estimate),0,0,'R');
$pdf->Cell(25,10,number_format($t_quoted),0,0,'R');
...
$pdf->SetFont('Arial','',11);
$pdf->Cell(40,10,'$ '.number_format($variation,2),0,0,'R');
...
$pdf->Output($filepath.'project_details.pdf','F');
$pdf->Output($filepath.'project_details.pdf','I');
}
?>