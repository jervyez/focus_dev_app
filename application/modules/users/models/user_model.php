...
AND UNIX_TIMESTAMP( STR_TO_DATE('$today', '%d/%m/%Y') ) <=  UNIX_TIMESTAMP( STR_TO_DATE( CONCAT (`seasons`.`date_finish`, '/$year') , '%d/%m/%Y') ) ");
	...
	public function fetch_user_logs(){
		$prev_year = date('Y');
		$date_last_year = '01/06/'.$prev_year;
	...
	public function inset_availability($user_id,$status,$notes,$date_time_stamp_a,$date_time_stamp_b,$reoccur_id = '0', $aus_timezone, $leave_req_id= '0'){
		...
		if ($if_has_availabilirt->num_rows >= 1) {

			foreach ($if_has_availabilirt->result() as $row){
	...
	// public function run_query_now(){
		
	// 	$query = $this->db->query("UPDATE `leave_request` SET `is_active` = 0, `is_disabled` = 1 WHERE `leave_request_id` = 921");
	// 	return $query;
	// }
}
