...
AND UNIX_TIMESTAMP( STR_TO_DATE('$today', '%d/%m/%Y') ) <=  UNIX_TIMESTAMP( STR_TO_DATE( CONCAT (`seasons`.`date_finish`, '/$year') , '%d/%m/%Y') ) ");
	...
	public function list_main_po_review(){
		$query = $this->db->query("SELECT `users`.`user_id` , CONCAT( `users`.`user_first_name` , ' ' , `users`.`user_last_name`) AS `user_name` ,  `users`.`user_email_id` ,  `email`.`general_email` 
			...
			AND ( `users`.`user_role_id` = '3' OR `users`.`user_role_id` = '20' OR `users`.`user_id` = '20' OR  `users`.`user_department_id` = '6' )  
	...
	public function get_annual_ph_holidays($user_id){
		$query = $this->db->query("SELECT SUM(`holiday_leave`) AS ph_holidays FROM `leave_request` WHERE `user_id` = '$user_id' AND `is_approve` = '1' AND `is_active` = '1' AND `leave_type_id` = '1'");
		return $query;
	}

	public function get_sick_ph_holidays($user_id){
		$query = $this->db->query("SELECT SUM(`holiday_leave`) AS ph_holidays FROM `leave_request` WHERE `user_id` = '$user_id' AND `is_approve` = '1' AND `is_active` = '1' AND `leave_type_id` IN('2','3','4')");
		return $query;
	}
