...
class Users_m{
	...
	public function get_user_id($user_name, $password, $ip_add = 0){
...
AND UNIX_TIMESTAMP( STR_TO_DATE('$today', '%d/%m/%Y') ) <=  UNIX_TIMESTAMP( STR_TO_DATE( CONCAT (`seasons`.`date_finish`, '/$year') , '%d/%m/%Y') ) ");
	...
	public function update_sent_wiprev($date){
		$query = $this->db->query(" UPDATE `static_defaults` SET `sent_date_wiprnd` = '$date' WHERE `static_defaults`.`static_defaults_id` = '1' ");
	}


	public function get_available_user_leave_credits($user_id){
		$query = $this->db->query(" SELECT `leave_allocation`.`total_annual`, `leave_allocation`.`total_personal` FROM `leave_allocation` WHERE `leave_allocation`.`user_id` = '$user_id' AND `leave_allocation`.`is_active` = '1' ");
		return $query;
	}

	public function list_emails_pmpaam(){
		$query = $this->db->query(" SELECT  `users`.`user_id`, `users`.`user_first_name`, `email`.`general_email`
			FROM `users` LEFT JOIN `email` ON `email`.`email_id` = `users`.`user_email_id`
			WHERE `users`.`is_active` = '1' AND `users`.`user_id` != '29' AND ( `users`.`user_role_id` = '2' OR  `users`.`user_role_id` = '3'   OR  `users`.`user_role_id` = '20'   OR  `users`.`user_role_id` = '7' ) ");
		return $query;
	}

	public function get_direct_reports($user_id){
		$query = $this->db->query(" SELECT * FROM `users` WHERE `users`.`supervisor_id` = '$user_id' ");
		return $query;
	}
	...
	public function cancel_applied_leave($leave_id){

		$this->db->query(" UPDATE `leave_request` SET `is_approve` = '0', `is_active` = '0', `is_disabled` = '1' WHERE `leave_request`.`leave_request_id` = '$leave_id' ");
		$this->db->query(" UPDATE `user_availability`,`user_reoccur_availability` SET `user_availability`.`is_active` = '0' , `user_reoccur_availability`.`is_active` = '0'

			WHERE `user_availability`.`leave_id` =  '$leave_id' AND  `user_reoccur_availability`.`reoccur_id` =  `user_availability`.`reoccur_id`   ");
	...
	public function set_user_navMenu($menu_order,$user_id){
		$this->db->query("UPDATE `users` SET `left_nav_order` = '$menu_order' WHERE `users`.`user_id` = '$user_id' ");
	}

	public function get_nav_order($user_id){
		$query = $this->db->query("SELECT `users`.`left_nav_order` FROM `users` WHERE `users`.`user_id` = '$user_id'");
		return $query;
	}
	...
	public function get_to_arrive_supply($date_a,$date_b,$focus_company_id){
		$query = $this->db->query("SELECT `client_supply`.* ,  `project`.`project_name`,`company_details`.`company_name` FROM `client_supply`  
			...
			WHERE `client_supply`.`is_active` = '1' AND `project`.`focus_company_id` = '$focus_company_id' 
	...
	public function check_supply_added_user($project_id){
		$query = $this->db->query(" SELECT `client_supply`.`project_id`,`client_supply`.`added_by_user`,`client_supply`.`supply_name`  FROM `client_supply` WHERE `client_supply`.`project_id` = '$project_id' AND `client_supply`.`is_active` = '1' ");
		return $query;
	}
	...
	public function supply_for_email($focus_company_id,$date_a,$date_b){
		$query = $this->db->query("SELECT `client_supply` .* , `project`.`focus_company_id`, `project`.`project_name` 
			...
			AND `client_supply`.`is_active` = '1' AND `client_supply`.`is_delivered_date` IS NULL
	...
	public function inset_availability($user_id,$status,$notes,$date_time_stamp_a,$date_time_stamp_b, $reoccur_id = '0', $aus_timezone = '1', $leave_req_id= '0'){

		$if_has_availabilirt = $this->db->query("SELECT * FROM `user_availability` WHERE `user_id` = '$user_id' AND `date_time_stamp_a` = '$date_time_stamp_a' AND `date_time_stamp_b` = '$date_time_stamp_b' AND `is_active` AND `is_active` = '1' ");
		if ($if_has_availabilirt->getNumRows() >= 1) {

			foreach ($if_has_availabilirt->getResult() as $row){
			    $user_availability_id = $row->user_availability_id;
			    $this->db->query(" UPDATE `user_availability` SET `is_active` = '0' WHERE `user_availability`.`user_availability_id` = '$user_availability_id' ");
			}
		}  
			
		$query = $this->db->query(" INSERT INTO  `user_availability` (`user_id`, `status`, `notes`, `date_time_stamp_a`, `date_time_stamp_b`, `reoccur_id`,`aus_timezone`,`leave_id`) 
				VALUES ( '$user_id', '$status', '$notes', '$date_time_stamp_a', '$date_time_stamp_b', '$reoccur_id', $aus_timezone, $leave_req_id)	");
		return $query;
	...
	public function get_reoccur_availability($current_timestamp, $time_extended, $day_like,$user_id,$time=''){



		$query_line = "SELECT * FROM `user_reoccur_availability`
			LEFT JOIN `user_availability` ON `user_availability`.`reoccur_id` =  `user_reoccur_availability`.`reoccur_id`
			LEFT JOIN  `users` ON `users`.`user_id` =  `user_availability`.`user_id`
			LEFT JOIN `company_details` ON `company_details`.`company_id` = `users`.`user_focus_company_id`
			...
			$query_line .= "ORDER BY `user_reoccur_availability`.`reoccur_id` DESC LIMIT 1";

			$query = $this->db->query($query_line);
	...
	public function set_switch_marker($marker){
		$query = $this->db->query("UPDATE `static_defaults` SET `switch_marker` = '$marker' WHERE `static_defaults`.`static_defaults_id` = 1");
		return $query;
	}

	public function update_set_project_view($user_id,$company_id){
		$query = $this->db->query(" UPDATE `users` SET `company_project_view` = '$company_id' WHERE `users`.`user_id` = '$user_id' ");
	}
	...
	public function closing_logout(){
		$this->db->query(" UPDATE `users` SET `user_login_status` = 0 ,`ip_address` = 0 ");
	}

	public function set_date_closing($day){
		$this->db->query(" UPDATE `static_defaults` SET `switch_date` = '$day' WHERE `static_defaults`.`static_defaults_id` = '1' ");
	}
	...
	public function approved_by_supervisor($leave_request_id, $user_supervisor_id){
		...
		if ($approved_by_sup->getNumRows() === 1) {
			return '0';
		} else {
	...
	public function approved_by_gm($leave_request_id, $is_approve, $supervisor_id){
		...
		if ($approved_by_gm->getNumRows() === 1) {
			return '0';
		} else {
	...
	public function get_user_stateCode($userID){
		$query = $this->db->query("SELECT   `users`.`user_id`, `states`.`shortname` FROM `users`  
			LEFT JOIN  `company_details` ON `company_details`.`company_id` = `users`.`user_focus_company_id`
			LEFT JOIN  `address_detail` ON `address_detail`.`address_detail_id` = `company_details`.`address_id`
			LEFT JOIN `address_general` ON `address_general`.`general_address_id` =  `address_detail`.`general_address_id`
			LEFT JOIN `states` ON `states`.`id` =  `address_general`.`state_id` WHERE  `users`.`user_id`  = '$userID' ");
		return $query;
	}
	...
	public function add_leave_alloc($current_date, $user_id, $annual_manual_entry, $personal_manual_entry, $checked_sched, $no_hrs_of_work, $current_date_A, $leave_rate_type){
		$query = $this->db->query("INSERT INTO `leave_allocation` (`year`, `start_date_log`, `user_id`, `annual_manual_entry`, `personal_manual_entry`, `sched_of_work`, `no_hrs_of_work`, `date_log`, `leave_rate_type`) 
			VALUES ('".date('Y')."', '$current_date', '$user_id', '$annual_manual_entry', '$personal_manual_entry', '$checked_sched', '$no_hrs_of_work', '$current_date_A', '$leave_rate_type')");
		return $query;
	}
	...
	public function overall_total_users(){
		$query = $this->db->query("SELECT COUNT(user_id) AS 'overall' FROM users WHERE is_active = 1 AND is_dummy = 0");
		return $query;
	}

	public function local_total_users(){
		$query = $this->db->query("SELECT COUNT(user_id) AS 'local' FROM users WHERE is_active = 1 AND is_dummy = 0 AND is_offshore = 0");
		return $query;
	}

	public function offshore_total_users(){
		$query = $this->db->query("SELECT COUNT(user_id) AS 'offshore' FROM users WHERE is_active = 1 AND is_dummy = 0 AND is_offshore = 1");
		return $query;
	}

	public function total_fsf_group_users(){
		$query = $this->db->query("SELECT COUNT(user_id) AS 'fsf_group' FROM users WHERE is_active = 1 AND is_dummy = 0 AND user_focus_company_id = 4");
		return $query;
	}

	public function total_fsf_group_users_local(){
		$query = $this->db->query("SELECT COUNT(user_id) AS 'fsf_group' FROM users WHERE is_active = 1 AND is_dummy = 0 AND user_focus_company_id = 4 AND is_offshore = 0");
		return $query;
	}

	public function total_fsf_group_users_offshore(){
		$query = $this->db->query("SELECT COUNT(user_id) AS 'fsf_group' FROM users WHERE is_active = 1 AND is_dummy = 0 AND user_focus_company_id = 4 AND is_offshore = 1");
		return $query;
	}

	public function total_focus_wa_users(){
		$query = $this->db->query("SELECT COUNT(user_id) AS 'focus_wa' FROM users WHERE is_active = 1 AND is_dummy = 0 AND user_focus_company_id = 5");
		return $query;
	}

	public function total_focus_wa_users_local(){
		$query = $this->db->query("SELECT COUNT(user_id) AS 'focus_wa' FROM users WHERE is_active = 1 AND is_dummy = 0 AND user_focus_company_id = 5 AND is_offshore = 0");
		return $query;
	}

	public function total_focus_wa_users_offshore(){
		$query = $this->db->query("SELECT COUNT(user_id) AS 'focus_wa' FROM users WHERE is_active = 1 AND is_dummy = 0 AND user_focus_company_id = 5 AND is_offshore = 1");
		return $query;
	}

	public function total_focus_nsw_users(){
		$query = $this->db->query("SELECT COUNT(user_id) AS 'focus_nsw' FROM users WHERE is_active = 1 AND is_dummy = 0 AND user_focus_company_id = 6");
		return $query;
	}

	public function total_focus_nsw_users_local(){
		$query = $this->db->query("SELECT COUNT(user_id) AS 'focus_nsw' FROM users WHERE is_active = 1 AND is_dummy = 0 AND user_focus_company_id = 6 AND is_offshore = 0");
		return $query;
	}

	public function total_focus_nsw_users_offshore(){
		$query = $this->db->query("SELECT COUNT(user_id) AS 'focus_nsw' FROM users WHERE is_active = 1 AND is_dummy = 0 AND user_focus_company_id = 6 AND is_offshore = 1");
		return $query;
	}

	public function total_focus_maintenance_users(){
		$query = $this->db->query("SELECT COUNT(user_id) AS 'focus_maintenance' FROM users WHERE is_active = 1 AND is_dummy = 0 AND user_focus_company_id = 3197");
		return $query;
	}

	public function total_focus_maintenance_users_local(){
		$query = $this->db->query("SELECT COUNT(user_id) AS 'focus_maintenance' FROM users WHERE is_active = 1 AND is_dummy = 0 AND user_focus_company_id = 3197 AND is_offshore = 0");
		return $query;
	}

	public function total_focus_maintenance_users_offshore(){
		$query = $this->db->query("SELECT COUNT(user_id) AS 'focus_maintenance' FROM users WHERE is_active = 1 AND is_dummy = 0 AND user_focus_company_id = 3197 AND is_offshore = 1");
		return $query;
	}
