...
class Users extends MY_Controller{
	...
	public function check_closing_time(){
		...
		if( $this->_is_logged_in() ){
			...
			if($switch_marker != $day_today){
				if($date_time_stamp_a < $time_now && $time_now < $date_time_stamp_b && !$this->session->userdata('set_alert')){
					$this->session->set_userdata('set_alert', 1 ); 
					echo '<script type="text/javascript">alert("'.$swtich_msg.'");</script>';

				}

				if($date_time_stamp_a < $time_now && $time_now < $date_time_stamp_b && $this->session->userdata('set_alert')){ // error !!! must occur only once
					$this->user_model->closing_logout();
	...
	public function showAvailableLeave($user_id){
		...
		if($q_user_leave->num_rows == 1){
			$leave_credits = array_shift($q_user_leave->result_array());
			$total_annual = $leave_credits['total_annual'];
			$total_personal = $leave_credits['total_personal'];
		}


		if($this->session->userdata('is_admin') != 1 &&  $this->session->userdata('user_id') == $user_id){
	...
	public function wip_remind_wipreview(){

		$static_defaults_q = $this->user_model->select_static_defaults();
		$static_defaults = array_shift($static_defaults_q->result_array());

		$day = $static_defaults['prj_review_day'];
		$time = $static_defaults['send_wip_rrtime'];
		$sent_date = $static_defaults['sent_date_wiprnd'];

		$sent_stamp = strtotime(date_format(date_create_from_format('d/m/Y', $sent_date), 'm/d/Y'));
...
*/
		...
		$end_time_set = $time_hour[0]+1;
		$time_end = $end_time_set.':00 '.$time_ampm[1];

		$start_date_remind = strtotime( $day." ".$time.' this week' );
		$end_date_remind = strtotime( $day." ".$time_end.' this week' );
		$current_timestamp = strtotime( "now" );
		$today_stamp =  strtotime( date('m/d/Y') );


 if($start_date_remind <= $current_timestamp && $current_timestamp <= $end_date_remind){
		...
		$emails_q= $this->user_model->list_emails_pmpaam();
		$send_emails = $emails_q->result();

		foreach($send_emails as $key => $email){
	//		echo "<p>$email->general_email</p>";
			$user_mail->addBcc($email->general_email);
		}

	if($today_stamp != $sent_stamp){
		$date_today = date('d/m/Y');
		$this->user_model->update_sent_wiprev($date_today);

		// send
		// update

		$user_mail->setFrom('donot-reply-sojourn@focusshopfit.com.au', 'Sojourn - Reminders');
		$user_mail->addAddress('','Sojourn User');    		 // Add a recipient
		//$user_mail->addAddress($user_email);               // Name is optional


		$user_mail->addBcc("jervy@focusshopfit.com.au");
	//	$user_mail->addBcc("jervyezaballa@gmail.com");
	//	$user_mail->addBcc("joan.kristine@yahoo.com");
	//	$user_mail->addBcc("katrina@focusshopfit.com.au");
	//	$user_mail->addBcc("ian@focusshopfit.com.au");

		$user_mail->addReplyTo('donot-reply-sojourn@focusshopfit.com.au', 'Sojourn - Reminders');
		$user_mail->isHTML(true);                                  // Set email format to HTML

		$year = date('Y');

		$user_mail->Subject = 'Weekly WIP Review';

		$msg_text = "<p style=\"font-family: Calibri,Candara,Segoe,Segoe UI,Optima,Arial,sans-serif; font-size: 16px;\">Please ensure that you complete the review of ALL projects that you have in the Sojourn WIP, when checking please ensure the following are all updated and the information for each is correct, current and relevant as it is the information provided for each project that drives the processes throughout our system for may other users, IF this information is not maintained or is incorrect, then others within the company will be using incorrect or outdated data to make their decisions.<br /><br />";
		$msg_text .= "The following must be checked, confirmed and updated if required:<br /><br /><strong>&bull; Site start date<br />&bull; Site completion date<br />&bull; Invoice dates (if you change the site start or completion dates you WILL need to update the invoice dates)<br />&bull; Complete the JOB BOOKS (invoicing) for all invoices that are outstanding<br />&bull; Project Notes, add notes to the project that will provide information to others that will explain what is happening.</strong><br /><br />";
		$msg_text .= "ALL of the above must be done weekly by the end of each Wednesday, with ALL items in WIP please.<br /><br />";

		$user_mail->Body = $msg_text."Click <a href=\"https://sojourn.focusshopfit.com.au/projects/projects_wip_review\" target=\"_blank\" title=\"Sojourn - WIP Review\"><strong>this link</strong></a> to go to Sojourn - WIP Review screen.<br /><br />Do not reply to this email.<br />&copy; FSF Group ".$year."</p>";
...
*/
		...
		if($user_direct_reportee->num_rows > 0){
			foreach ($user_direct_reportee->result_array() as $row){
	...
	//	$user_mail->isSMTP();                                      		// Set mailer to use SMTP
		...
		if(isset($_POST['is_form_submit'])){
			...
			$is_dummy = $this->input->post('is_dummy', true);
...
*/
		...
		$this->form_validation->set_rules('is_dummy', 'Dummy Account','trim|required|xss_clean');
...
*/
			...
			$contractor_employee = $this->input->post('contractor_employee', true);
			$is_dummy = $this->company->if_set($this->input->post('is_dummy', true));
...
// PHPMailer class
 ...
 redirect('/users', 'refresh');
	...
	$this->wip_remind_wipreview(); /////// testing auto load for checking sendout
...
//$reminder_lead_days = 5;
						...
						if($date_b_name == 'Sun' || $date_b_name == 'Sat'){
					...
					//	echo "$comp_id,$date_a,$date_b  ___ $reminder_lead_days";
						...
						$get_to_arrive_supply_q = $this->user_model->get_to_arrive_supply($date_a,$date_b,$comp_id);
						...
						if ($get_to_arrive_supply_q->num_rows > 0 ) {
							...
							foreach ($get_to_arrive_supply_q->result() as $supply_rcv){
								...
								//echo "<br /><br />send inbound email ".$supply_rcv->supply_name.'';
							}
...
*/
						...
						//		echo "<br /><br />send outbound email ".$supply->supply_name.'';
...
//endif;
				...
				//	if( $userdata->user_id == 2){
						...
						//	$mail->addAddress($send_to);
                          ...
                          $mail->addCC('jervy@focusshopfit.com.au');
					...
					//	else{ // remove else after
						...
						if($userdata->skip_dashboard == 1){ 
							...
							if($this->session->userdata('user_role_id') == 8){
...
string(74) "19/07/2021 07:00 AM`19/07/2021 05:00 PM`TETS`2`/focus/users/account/2`Busy" 

string(68) "07:00 AM`05:00 PM`daily``mon,tue,thu,wed,fri`19/07/2021`23/07/2021`0"
...
*/
		...
		$user_state = array_shift($q_user_loc->result_array());
...
*/
 ...
 // echo "<p>$user_id,$status,$notes,$time_stamp_a,$time_stamp_b,$reoccur_id,$timezone<p>";
	...
	//	$nowTimeStamp = 1627516800;
		...
		if($user_id == 27){
			//$returnMsg = 'Indefinite Return.';
			//$returnMsg = ' Return by: Thursday 28/04/2022 8:00 AM';
			return $returnMsg;
		}
...
// regular availability
		...
		//	echo '<pre>';var_dump($table_arr_cell );echo '</pre>'; 
		...
		foreach ($table_arr_cell as $key => $value):
			...
			if( $table_arr_cell[$key]['isWeekEnd'] == 0 && $table_arr_cell[$key]['endTime'] == 0 ){  // shows next day as return day
				...
				$returnMsg = '*1*** Return by: '.$table_arr_cell[$key]['nameDay'].' '.$table_arr_cell[$key]['dateTimeStart'].' 8:00 AM'.''; // steve here error
				//echo $returnMsg;
				//$returnMsg = ' ???? ';
 				break;
			}
			...
			elseif( $table_arr_cell[$key]['endTime'] < 1700 && $table_arr_cell[$key]['endTime'] > 0 && $table_arr_cell[$key]['isWeekEnd'] == 0  ){
				...
				//	var_dump($table_arr_cell[$key],$table_arr_cell[$key]['isWeekEnd'], $table_arr_cell[$key]['is_free']);

	#			if($table_arr_cell[$key]['is_free'] == 1){
	#				echo '<pre> zzz ';
	#				echo date('l', strtotime('January 1st '.$year.' +'.$dayOfYear.' days'));
	#			echo '</pre>';
	#			}
...
*/
	...
	function get_user_availability($user_id,$mod='',$return_text=''){
...
function get_user_ave_comments($user_id,$return_text=''){
...
/*
			...
			if( $user_id == $this->session->userdata('user_id') ){
				...
				$loc_time_zone = $loc_data->time_zone;
			//	date_default_timezone_set($loc_time_zone);


				$state_code_loc = $loc_time_zone;
...
// $this->set_availability($availability_init);
	...
	public function update_menu_order(){
		...
		$menu_list_order = $this->input->post('menu_list_order');
	...
	public function get_user_nav_order($user_id){
			...
			$nav_order = array_shift($q_nav_menu->result() );
