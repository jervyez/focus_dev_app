...
//			
			...
			if(isset($work_contractor->ex_gst)){
				// go to completed
...
//		var_dump($_FILES);
		...
		$this->generate_CQR($project_id,$work_id,$works_contrator_id,FALSE,'stored_docs',1);
 ...
 */
		...
		$file_name = $project_id.'_cqr_reply_'.$work_id.'_'.$file_company_name.'_'.$works_contrator_id.'.pdf';
		
		$this->etc_m->insert_doc_cqr($file_name,$project_id,$date_upload);   
        ...
        if($has_attachment == 1){
        	$cpo_reply_email .= '<br />&bull; This job has downloadable attachments, click <a href="'.base_url().'project_attachments/proj_attachment?project_id='.$project_id.'" class="" id="" target="_blank" tile="Download project attachments"><strong id="" class="">this link</strong></a> to view.';
        }

        $cpo_reply_email .= '<br />&bull; Download the Completed CQR (Contractor Quote Request) form, click <a href="'.base_url().'docs/stored_docs/'.$file_name.'" class="" id="" target="_blank" tile="Download Contractor Purchase Order Form"><strong id="" class="">this link</strong></a> to view.</p>';
...
// file of the accomplished CQR here!!!!
		...
		$cpo_reply_email .= '<img src="'.base_url().'uploads/misc/'.$user_signature->banner_name.'" width="788" height="170" alt="Focus Shopfit PTY LTD Signature" />';
...
*/
		 ...
		 // 	echo '<p id="" class="">'.$file_name_amp.'</p>';
	 			...
	 			//	echo '<p>'.$has_error.'</p>'; 
        			...
        			$this->etc_m->insert_doc_cqr($file_name_amp,$project_id,$date_upload); //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	 			}
        	}
...
*/
	...
	public function contractor_quote_form($work_contractor_supplier_id){
...
*/
			...
			$this->generate_CQR($project_id,$row->works_id,$work_contractor_id);
			...
			$button_var = '<h2><a href="'.base_url().'submit_quote/_work_con_id_" style="padding: 8px 10px; text-decoration: none; border-radius: 5px; border: 1px solid #c8c6c4; color: #4f6bed; background-color: #EFEFEF;" target="_blank" title="Online Quotation Form">Online Quotation Form</a></h2>';
			...
			$file_name = $project_id.'_cqr_'.$row->works_id.'_'.$file_company_name.'_'.$work_contractor_id.'.pdf';
...
*/
			...
			if($has_attachment == 1){
				$cqr_email_template .= '<br />&bull; This project has downloadable attachments, click <a href="'.base_url().'project_attachments/proj_attachment?project_id='.$project_id.'" class="" id="" target="_blank" tile="Download project attachments"><strong id="" class="">this link</strong></a> to view.';
			}

	 

			$cqr_email_template .= '<br />&bull; Download the CQR (Contractor Quote Request) form, click <a href="'.base_url().'docs/stored_docs/'.$project_id.'_cqr_'.$row->works_id.'_'.$file_company_name.'_'.$work_contractor_id.'.pdf" class="" id="" target="_blank" tile="Download Contractor Quote Request Form"><strong id="" class="">this link</strong></a> to view.</p>';
			...
			$cqr_email_template .= '<img src="'.base_url().'uploads/misc/'.$user_signature->banner_name.'" width="788" height="170" alt="Focus Shopfit PTY LTD Signature" />';
...
//		var_dump($this->session->userdata);
 	...
 	redirect(base_url().'projects/view/'.$project_id.'?tab=works');  ////!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11
	...
	//	echo '<p class="">'.$day_today.' --- '.$project_id.' --- '.$today_date.'</p>'; 
		...
		//	echo '<p class="">'.$today_time.' ------ '.$works_contractor->work_replyby_time.' ------ '.$difference.'</p>'; 
			...
			<h2><a href="'.base_url().'submit_quote/'.$works_contractor->work_contractors_id.'" style="padding: 8px 10px; text-decoration: none; border-radius: 5px; border: 1px solid #c8c6c4; color: #4f6bed; background-color: #EFEFEF;" target="_blank" title="Online Quotation Form">Online Quotation Form</a></h2><p>';


			if($has_attachment == 1){
				$message .= '<br />&bull; This project has downloadable attachments, click <a href="'.base_url().'project_attachments/proj_attachment?project_id='.$works_contractor->project_id.'" class="" id="" target="_blank" tile="Download project attachments"><strong id="" class="">this link</strong></a> to view.';
				$has_attachment = 0;
			}

			$message .= '<br />&bull; Download the CQR (Contractor Quote Request) form, click <a href="'.base_url().'docs/stored_docs/'.$works_contractor->project_id.'_cqr_'.$contractor_supplier->works_id.'_'.$file_company_name.'_'.$contractor_supplier->works_contrator_id.'.pdf" class="" id="" target="_blank" tile="Download Contractor Quote Request Form"><strong id="" class="">this link</strong></a> to view.</p>';
			...
			<img src="'.base_url().'uploads/misc/'.$user_signature->banner_name.'" width="788" height="170" alt="Focus Shopfit PTY LTD Signature" />';
...
//	 echo '<p class="">*******************</p>'.$email_msg.'<p class="">*******************</p>'; 
	 ...
	 $subject = 'Tender Reminder: '.$client_company->company_name.' - '.$works_contractor->project_id.' '.$works_contractor->project_name.' - '.$contractor_supplier->company_name;
	...
	// echo '<p class="">'.$from.' --- '.$to.' --- '.$subject.'</p>'; 
	 ...
	 // •	Tender Reminder: OPSM Pty Ltd, Toronto NSW - 43684 Toronto NSW Upgrade - S2S Commercial Floors Pty. Ltd.
	 //  [purpose]: [client_name] - [project_number] [project_name] - [contractor_name]
...
//	 $subject = 'Focus Shopfit Tender not Received - '.$client_company->company_name;
 ...
 //  [purpose]: [client_name] - [project_number] [project_name] - [contractor_name]
	...
	public function set_send_email($from,$to,$subject,$msg,$arr_cc=array(),$arr_bcc=array()){
		...
		$mail->setFrom($from,'Focus Shopfit Tender');
		$mail->addAddress($to);
		$mail->addReplyTo($from,'Focus Shopfit Tender');
	...
	public function generate_SiteSheet($project_id='',$work_id='',$work_contractor_id='',$stream=FALSE, $dir='mss'){
	...
	if($contractor_supplier->is_pending == 1){
		...
		$maintenance_contact = array_shift($q_get_maintenance_contact->result());
	 	...
	 	<table width="100%" class="mrg-bottom--10">
	 		<tr>
	 			<td rowspan="2" width="30%"><img class="mrg-left-10 mrg-top-25" src="'.base_url().'img/focus_logo_cqr.jpg" width="250px" height="82.67px" /></td>
	 			<td width="35%">
	 	...
	 	<div id="" class="content_area pad-10 block heigh-auto">
		 	<table width="100%">
			 	...
			 	<tr>
			 		...
			 		<td><img class="mrg-top-5" src="'.base_url().'img/box_ballot.jpg" width="10px" height="10px" /> &nbsp; <img class="mrg-top-5" src="'.base_url().'img/box_ballot.jpg" width="10px" height="10px" /></td>
	 	...
	 	<div id="" class="content_area pad-10 block heigh-auto">
		 	<table width="100%">
			 	...
			 	<tr>
			 		...
			 		<td><p class=""><strong class="">Yes</strong> <img class="mrg-top-5" src="'.base_url().'img/box_ballot.jpg" width="10px" height="10px" /></p></td>
			 		<td><p class=""><strong class="">No</strong> <img class="mrg-top-5" src="'.base_url().'img/box_ballot.jpg" width="10px" height="10px" /></p></td>
			 		<td></td>
			 	</tr>
	 ...
	 <p class="text-right">Issued by: '.$estimator_contact->user_full_name.'</p>
		...
		$file_name = $project_id.'_maintenance_site_sheet_'.$work_contractor_id;
		$this->reports->generate_cqr_pdf($html['contents'],'portrait','A4',$file_name,$dir,$stream);
	 					...
	 					<div id="" class="pad-bottom-15">
	 						<div id="" class="clearfix block" style="height: 90px;">
	 							<div id="" class="pull-left width-50">
	 								<p id="" class="">
	 									'.($contractor_supplier->site_inspection_req == '0' ? '<img class="mrg-top-5" src="'.base_url().'img/box_ballot.jpg" width="10px" height="10px" />' : '<img class="mrg-top-5" src="'.base_url().'img/box_check.jpg" width="10px" height="10px" />').' &nbsp; Site Inspection Required<br />
	 									'.($contractor_supplier->special_conditions == '0' ? '<img class="mrg-top-5" src="'.base_url().'img/box_ballot.jpg" width="10px" height="10px" />' : '<img class="mrg-top-5" src="'.base_url().'img/box_check.jpg" width="10px" height="10px" />').' &nbsp; Special Condition<br />
	 									'.($contractor_supplier->additional_visit_req == '0' ? '<img class="mrg-top-5" src="'.base_url().'img/box_ballot.jpg" width="10px" height="10px" />' : '<img class="mrg-top-5" src="'.base_url().'img/box_check.jpg" width="10px" height="10px" />').' &nbsp; Additional Visit Required<br />
	 									'.($contractor_supplier->operate_during_install == '0' ? '<img class="mrg-top-5" src="'.base_url().'img/box_ballot.jpg" width="10px" height="10px" />' : '<img class="mrg-top-5" src="'.base_url().'img/box_check.jpg" width="10px" height="10px" />').' &nbsp; Operate During Install<br />
	 									'.($contractor_supplier->other == '0' ? '<img class="mrg-top-5" src="'.base_url().'img/box_ballot.jpg" width="10px" height="10px" />' : '<img class="mrg-top-5" src="'.base_url().'img/box_check.jpg" width="10px" height="10px" />').' &nbsp; Others
	 								</p>
	 							</div>
	 							<div id="" class="pull-left width-50">
	 								<p id="" class="mrg-left-20">
	 									'.($contractor_supplier->week_work == '0' ? '<img class="mrg-top-5" src="'.base_url().'img/box_ballot.jpg" width="10px" height="10px" />' : '<img class="mrg-top-5" src="'.base_url().'img/box_check.jpg" width="10px" height="10px" />').' &nbsp; Week Work<br />
	 									'.($contractor_supplier->weekend_work == '0' ? '<img class="mrg-top-5" src="'.base_url().'img/box_ballot.jpg" width="10px" height="10px" />' : '<img class="mrg-top-5" src="'.base_url().'img/box_check.jpg" width="10px" height="10px" />').' &nbsp; Weekend Work<br />
	 									'.($contractor_supplier->after_hours_work == '0' ? '<img class="mrg-top-5" src="'.base_url().'img/box_ballot.jpg" width="10px" height="10px" />' : '<img class="mrg-top-5" src="'.base_url().'img/box_check.jpg" width="10px" height="10px" />').' &nbsp; After Hours Work<br />
	 									'.($contractor_supplier->new_premises == '0' ? '<img class="mrg-top-5" src="'.base_url().'img/box_ballot.jpg" width="10px" height="10px" />' : '<img class="mrg-top-5" src="'.base_url().'img/box_check.jpg" width="10px" height="10px" />').' &nbsp; New Premises<br />
	 									'.($contractor_supplier->free_access == '0' ? '<img class="mrg-top-5" src="'.base_url().'img/box_ballot.jpg" width="10px" height="10px" />' : '<img class="mrg-top-5" src="'.base_url().'img/box_check.jpg" width="10px" height="10px" />').' &nbsp; Free Access
	 								</p>
	 							</div>
	 						</div>
...
*/
		...
		$file_name = $project_id.'_'.$cqr_reply.'_'.$work_id.'_'.$file_company_name.'_'.$work_contractor_id;
		$this->reports->generate_cqr_pdf($html['contents'],'portrait','A4',$file_name,$dir,$stream);
		$date_today = date('d/m/Y');
...
*/
	...
	//		echo '<p id="" class="">'.$project_id.' --- '.$row->works_id.' --- '.$company_contractor_supplier_id.' --- '.$row->is_pending.'--- '.$work_contractor_id.'</p>'; 
			...
			$this->generate_CPO($project_id,$row->works_id,$work_contractor_id);
if($row->job_category == 'Maintenance'){
		...
		//	$subject = $project_id.' '.$row->project_name.' CPO - '.$row->company_name;
			...
			$cpo_email = '<p class="">Greetings,<br /></p><p>Please see the link below for the Purchase Order Form, we would appreciate if you confirmed receipt. Kindly review the details of the linked documents for your reference.</p><p class="">If you have any questions or there is any other information required, please contact me through '.$from.'. <br />';
			$cpo_email .= 'We hope for a long business relationship with your company.</p><p>';

			if($has_attachment == 1){
				$cpo_email .= '<br />&bull; This job has downloadable attachments, click <a href="'.base_url().'project_attachments/proj_attachment?project_id='.$project_id.'" class="" id="" target="_blank" tile="Download project attachments"><strong id="" class="">this link</strong></a> to view.';
			}
if($row->job_category == 'Maintenance'){
			if($set_attach_mss == 1){
				$cpo_email .= '<br />&bull; Download the MSS (Maintenance Site Sheet) form, click <a href="'.base_url().'docs/mss/'.$project_id.'_maintenance_site_sheet_'.$work_contractor_id.'.pdf" class="" id="" target="_blank" tile="Download Maintenance Site Sheet Form"><strong id="" class="">this link</strong></a> to view.';
			}
 
}
			

			$cpo_email .= '<br />&bull; Download the CPO (Contractor Purchase Order) form, click <a href="'.base_url().'docs/stored_docs/'.$project_id.'_cpo_'.$row->works_id.'_'.$file_company_name.'_'.$work_contractor_id.'.pdf" class="" id="" target="_blank" tile="Download Contractor Purchase Order Form"><strong id="" class="">this link</strong></a> to view.</p>';
			...
			$cpo_email .= '<img src="'.base_url().'uploads/misc/'.$user_signature->banner_name.'" width="788" height="170" alt="Focus Shopfit PTY LTD Signature" />';


			$cpo_file_name = $project_id.'_cpo_'.$row->works_id.'_'.$file_company_name.'_'.$work_contractor_id.'.pdf';
...
//Induction Video Link Email ==========================
			...
			$footer_email .= '<img src="'.base_url().'uploads/misc/'.$user_signature->banner_name.'" width="788" height="170" alt="Focus Shopfit PTY LTD Signature" />';
							...
							if($video_generated == 1):
								if($focus_company_id == 6){
									...
									//	$mail->addBCC($sender_user_email);
										...
										$mail->Subject = "Induction Video Link";
...
//Induction Video Link Email ==========================
 ...
 redirect(base_url().'projects/view/'.$project_id.'?tab=works');  ////!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11
	...
	public function generate_CPO($project_id='',$work_id='',$work_contractor_id='',$stream=FALSE, $dir='stored_docs',$contractor_reply='0'){
