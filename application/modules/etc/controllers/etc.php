...
class Etc extends MY_Controller{
	...
	//	var_dump($_GET);
		...
		$this->session->set_userdata('work_contractor_supplier_id', $work_contractor_supplier_id);
...
//			
			...
			if($work_contractor->prj_quote_review == 1){
				$this->session->set_userdata('work_contractor_supplier_id', $work_contractor_supplier_id);
				redirect('/contractor_quote_form/'.$work_contractor_supplier_id);
				exit;
			}
...
//		var_dump($_FILES);
		...
		$is_quote_review = $this->input->post('is_quote_review', true); //----
		$old_quoted_amnt = str_replace(',','',  $this->input->post('old_quoted_amnt', true) );


//////////////////////////// here
		$contector_action = '';
		if($old_quoted_amnt != $ex_gst ){
			$contector_action = 'Contractor submitted quote review and changed the quote from $'.number_format($old_quoted_amnt,2).' to $'.number_format($ex_gst,2);
			if($is_quote_review == 1){

				$this->etc_m->quote_review_ok($works_contrator_id);
				$this->etc_m->insert_quote_review($works_contrator_id, $work_id, $contractor_id, $date, $ex_gst, $project_id);
				
			}

		}else{
			$contector_action = 'Contractor submitted quote review and remains at $'.number_format($old_quoted_amnt,2);
		}

		$this->etc_m->remove_quote_rev($works_contrator_id);
		...
		$time = date("H:i:s");
		$type = "INSERT";
		$actions = $contector_action;
		$this->etc_m->insert_user_log($user_id,$date,$time,$actions,$project_id,$type);
		// user logs
...
//$file_company_name = strtolower( str_replace(' ', '_',  $work_contractor_data_arr['company_name']    )  );
 ...
 */
  ...
  // 	var_dump($data);
		...
		$pdf_file = $project_id.'_cqr_'.$work_contractor_data_arr['works_id'].'_'.$file_company_name.'_';
		$pdf_file_cqr = $this->pull_out_file($pdf_file);
		$data['cqr_file_pdf'] = $pdf_file_cqr;



		$pdf_file_list = $project_id.'_cqr_reply_'.$work_contractor_data_arr['works_id'].'_'.$file_company_name.'_';
		$pdf_file_reply_cqr = $this->list_contractor_attached_files($pdf_file_list);
		$data['pdf_file_list'] = $pdf_file_reply_cqr;
...
*/
	...
	public function confirm_review($work_contractor_id=''){

		if( isset($work_contractor_id) && $work_contractor_id != '' ){
			...
			$time = date("H:i:s");
			$type = "INSERT";
			$actions = 'Contractor kept the original quote for ID: '.$work_contractor_id;
			$this->etc_m->insert_user_log($user_id,$date,$time,$actions,$project_id,$type);
			// user logs

			$this->etc_m->remove_quote_rev($work_contractor_id);
			redirect(base_url().'confirm_review');
		}else{
			$this->load->view('done_review');
		}
...
*/
			...
			$button_var = '<h2><a href="'.base_url().'submit_quote/_work_con_id_?cqr_file=_pdf_cqr_file_" style="padding: 8px 10px; text-decoration: none; border-radius: 5px; border: 1px solid #c8c6c4; color: #4f6bed; background-color: #EFEFEF;" target="_blank" title="Online Quotation Form">Online Quotation Form</a></h2>';
...
*/
			...
			if($row->prj_quote_review == 1):
				$cqr_email_template .= '<p class="">&nbsp;</p><p><strong class="">If you decided not to change your quote, you can click the button below.</strong></p>';
				$cqr_email_template .= '<h2><a href="'.base_url().'confirm_review/'.$row->works_contrator_id.'" style="padding: 8px 10px; text-decoration: none; border-radius: 5px; border: 1px solid #c8c6c4; color: #ffffff; background-color: #8479ff;" target="_blank" title="Keep Quotation">Keep My Original Quote</a></h2>';
				$cqr_email_template .= '<p class="">';

				$pdf_file = $row->project_id.'_cqr_reply_'.$row->works_id.'_'.$file_company_name.'_';
				$pdf_file_reply_cqr = $this->list_contractor_attached_files($pdf_file);

				if(count($pdf_file_reply_cqr) > 0){

					$cqr_email_template .= '<strong class="">Review your previously accomplished CQR (Contractor Quote Request) form and other files relating to this quote.</strong>';

					foreach ($pdf_file_reply_cqr as $key => $value) {
						$cqr_email_template .= '<br />&bull; <a href="'.base_url().'docs/stored_docs/'.$value.'" target="_blank" title="'.$value.'" >'.$value.'</a>';
					}

					$cqr_email_template .= '<br />';
				}			 
			...
			$cqr_email_template .= '<br />&bull; Download the CQR (Contractor Quote Request) form, click <a href="https://sojourn.focusshopfit.com.au/docs/stored_docs/'.$file_name.'" class="" id="" target="_blank" tile="Download Contractor Quote Request Form"><strong id="" class="">this link</strong></a> to view.';



			$cqr_email_template .= '</p><p>';
			if(!empty($extra_msg)){
...
*/
	...
	//		$subject = 'Focus Shopfit Tender - '.$client_company->company_name.', '.ucwords(strtolower($site_address->suburb)).', '.$site_address->shortname.' - '.$project_id;
			...
			if($row->prj_quote_review == 1):
				$subject = 'Contractor Quotation Review: '.$client_company->company_name.' - '.$project_id.' '.$row->project_name.' - '.$contractor_name;
			else:
...
$pdf_file = $works_contractor->project_id.'_cqr_'.$contractor_supplier->works_id.'_'.$file_company_name.'_'; // day before
$pdf_file_cqr = $this->pull_out_file($pdf_file);


//echo '<pre>';var_dump($pdf_file, $pdf_file_cqr );echo '</pre>'; 
...
//////////////  for pdf contractor
	...
	//	$cc_email_arr = explode(',',$set_cc_emails);
		...
		$q_user_sender_contact = $this->etc_m->fetch_user($user_id);
		$user_sender_contact = array_shift($q_user_sender_contact->result());
...
$cpo_file_name =  str_replace( array( "'",  '"', ',', '"' , '`'  ,'%' ,  '&apos', ' ' , ';', '&', '&apos', '<', '>','/' ), '', $cpo_file_name);
			...
			$bcc_user_sender = $user_sender_contact->general_email;
/*
			if($user_role_id != 7 && $user_role_id != 2 && $user_role_id != 3){
				array_push($bcc_email_arr,$bcc_user_sender);
			}
*/

			
			$emails_grouped = array_merge($cc_email_arr, $bcc_email_arr);


			if($row->project_manager_id != 29){
				if (!in_array($bcc_user_sender, $emails_grouped)) {
					//if($user_role_id != 7 && $user_role_id != 2 && $user_role_id != 3){
						array_push($bcc_email_arr,$bcc_user_sender);
					//}
				}
...
/*
				 		 foreach ($arr_details_lines as $key => $value) {
				 		 	...
				 		 	if(strlen($value) > 115){
				 		 		$html['contents'] .= substr($value,0,115).'<br />';
				 		 		$html['contents'] .= substr($value,115,115).'<br />';
				 		 		$html['contents'] .= substr($value,230,115).'<br />';
...
$file_name =  str_replace( array( "'",  '"', ',', '"' , '`'  ,'%' ,  '&apos', ' ' , ';', '&', '&apos', '<', '>','/' ), '', $file_name);
	...
	public function set_email_notif(){
		$user_id = $this->session->userdata('user_id');
	...
	public function pull_out_file($file_lookOut=''){

		$file_name = '';

	//	$file_lookOut = '45775_cqr_350561_premier_remedial_building_';
		$dirUserPath = dirname(dirname(dirname(dirname(dirname(__FILE__))))). "/docs/stored_docs";

		$dirAdmin = glob($dirUserPath ."/". $file_lookOut."*");
	...
	public function list_contractor_attached_files($file_lookOut=''){
	...
	public function test_file_load($file_lookOut=''){
		var_dump( $this->pull_out_file($file_lookOut) );
	}
}