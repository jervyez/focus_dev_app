								...
								<div class="box-area pad-10 clearfix">	
									...
									<div class="box m-bottom-15 clearfix">
										<div class="box-head pad-5 m-bottom-5">
											...
											<a href="<?php echo base_url(); ?>projects/view/<?php echo $project_id; ?>" class="btn btn-success btn-xs pull-right" role="button"><i class="fa fa-hand-o-left"></i> Return to Project Details</a>
										</div>
										...
										<div class="box-area pad-5 clearfix">
											...
											<div class="col-md-12 pad-10" style="margin: 20px 0;">
												<h4 style="margin-bottom: 40px;"><i class="fa fa-file-image-o"></i> Uploaded Images: <small>(After done editing photos, Select Photos that you want to include in the PDF.) - </small> 
													<small><button type="button" id="selectImageGallery" class="btn btn-link tooltip-test" style="margin: 0;padding: 0; color: #245580;" title="Select photos that you want to include in the PDF."><strong>[Select Photos]</strong></button></small>
													<small><button type="button" id="editImageGallery" class="btn btn-link tooltip-test" style="margin: 0;padding: 0; color: #ea6503;" title="Crop, Rotate, Put labels and Select Group on your Photos."><strong>[Edit Photos]</strong></button></small> 
													<small><button type="button" id="deleteAllimage" class="btn btn-link tooltip-test" style="margin: 0;padding: 0; color: #d9534f;" title="Delete all photos in one click."><strong>[Delete All Photos]</strong></button></small>
													<small><button type="button" id="selectAllimage" class="btn btn-link tooltip-test" style="margin: 0;padding: 0; color: #5bc0de;" title="Select All the Photos you want to include in the PDF"><strong>[Select All Photos]</strong></button></small> 
												</h4>
											...
											<div class="col-md-11 pad-10 m-top-20">
												...
												<div class="col-md-2 m-bottom-5">
													<form action="<?php echo base_url(); ?>projects/do_upload/<?php echo $project_id ?>" method="post" enctype="multipart/form-data" id="addphotos_frm">
														...
														<span class="btn btn-primary btn-block btn-file">
													    	<i class = "fa fa-plus-circle"></i> Add Photos<input type="file" id="addphotos_btn" name="userfile[]" multiple="multiple" accept="image/*">
														</span>
													</form>
												</div>
											</div>
						...
						<div class="col-md-3">
							<div class="box">
								...
								<div class="box-area pad-5">
									<ul>
										<li>After editing photos, click the 'Select Photos', note that the images are clickable, select images you want to include in the Progress Report for generation.</li>
										<li>'Update Details' button are for only Leading Hand, Construction Manager and Works Progress Details saving process, Images are updated dynamically.</li>
									</ul>
								</div>
							</div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#deleteAllimage').click(function(){

			var project_id = $('#project_id').val();

			$('#confirmModal').modal('show');

		    $('#confirmText').text('Are you sure you want to delete all the photos?');
	    	$('#confirmButtons').html('<button type="button" class="btn btn-danger" data-dismiss="modal">No</button>' +
	                    			  '<button type="button" class="btn btn-success" onclick="confirmDelAllImages('+project_id+');">Yes</button>');
		});
		...
		$('#selectAllimage').click(function(){
		    ...
		    $('#confirmText').text('Are you sure you want to select all the photos?');
	    	$('#confirmButtons').html('<button type="button" class="btn btn-danger" data-dismiss="modal">No</button>' +
	                    			  '<button type="button" class="btn btn-success" onclick="confirmSelAllImages('+project_id+');">Yes</button>');
		});
					...
					type: 'GET',
			      	...
			      	success: function (response) {
			         	if( response.status === true ) {
			             	...
			             	$("li#image_report_"+img_id+" .imgWrapper > img").prop("src", "<?php echo base_url(); ?>uploads/project_progress_report/"+project_id+"/"+img_name+"?" + +new Date());			             	
		...
		function saveImageLbl(last_img_name, img_name){
			...
			$.ajax({
				...
				'data' : {'ajax_var' : data },
				'success' : function(data){
					var img_name_sliced = img_name_cut.slice(0, -4);
		...
		$("#btnAttachSend").click(function(){
			...
			var pr_path = rootFolder+'/reports/project_progress_report/'+project_id+'/'+project_id+' '+project_name+' Progress Report '+pr_version+'.pdf'; // live
			// var pr_path = rootFolder+'/public_html/reports/project_progress_report/'+project_id+'/'+project_id+' '+project_name+' Progress Report '+pr_version+'.pdf'; // local
		...
		$('#sendpdf_btn').click(function(){
			...
			var sendpdf_path = rootFolder+'/reports/project_progress_report/'+project_id+'/'+project_id+' '+project_name+' Progress Report '+pr_version+'.pdf'; // live
			// var sendpdf_path = rootFolder+'/public_html/reports/project_progress_report/'+project_id+'/'+project_id+' '+project_name+' Progress Report '+pr_version+'.pdf'; // local

			var data = sendpdf_main_to+'|'+sendpdf_other_emails+'|'+sendpdf_from+'|'+sendpdf_from_name+'|'+sendpdf_subject+'|'+sendpdf_body+'|'+sendpdf_path+'|'+sendpdf_cc+'|'+sendpdf_bcc; 
	...
	function set_edited(img_name, param){
		...
		if (img_name.lastIndexOf("?") != -1){
			...
			data = img_name_cut+'|'+project_id+'|'+param; 
			...
			data = img_name+'|'+project_id+'|'+param; 
	...
	function getGroupImg(img_name){
		...
		data = img_name_cut+'|'+project_id; // alert(data);
	...
	function updateGroupImg(img_name, image_name_trim){
		...
		data = img_name_cut+'|'+project_id+'|'+group_selected_id; 
	...
	function open_pr_pdf(pr_version){
		...
		var file_name = project_id+' '+project_name+' Progress Report '+pr_version+'.pdf';
	...
	$("input:file#addphotos_btn").change(function(){
		$("#addphotos_frm").submit();
	});

	function confirmDelAllImages(project_id){

		var data = project_id;

		ajax_data(data,'projects/delete_all_images');
		alert('Successfully Deleted All Images!');
		
		location.reload(true);
	}

	function confirmSelAllImages(project_id){

		var data = project_id;

		ajax_data(data,'projects/select_all_images');
		alert('Successfully Selected All Images!');
