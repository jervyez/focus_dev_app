...
*/
	...
	$site_add .= $site_address->unit_number.' '.$site_address->street.'<br />'. ucwords(strtolower($site_address->suburb)).', '.$site_address->shortname.', '.$site_address->postcode;
 	...
 	if(   $prj_conshnd->leading_hand_id == 0 &&   $prj_conshnd->contruction_manager_id == 0 ){
 		$prj_conshnd->leading_hand_id = 999;
 		$leading_hand_details->user_full_name = $prj_conshnd->lh_name;
 		$leading_hand_details->mobile_number = $prj_conshnd->lh_contact;
 	}
...
// page 18
	...
	if (file_exists($qr_code)) {
		$content .= '<center><span class="" style="text-align: center; font-weight:bold; text-decoration: underline; font-size: 34px;  color: #000;">Site Specific induction QR Code</span><br /><p class="" style="font-size:10px !important; text-align: center;"><br />Scan Code to view the Site Specific induction.<br />Please note you have had to have completed the Focus Shopfit induction prior to doing this site specific induction.</p><img align="center" src="'.$qr_code.'" width="280px" height="280px"  /><center><p id="" class="">&nbsp;</p>';
	}




$content .= '<p id="" class="">&nbsp;</p>

<p>';


	if (file_exists($qr_siteDiary)) {
		$content .= '<center><span class="" style="text-align: center; font-weight:bold; text-decoration: underline; font-size: 34px;  color: #000;">Site Sign-in QR Code</span><br /><img align="center" src="'.$qr_siteDiary.'" width="280px" height="280px"  /><center>';
	}



$content .= '<center style="font-size:10px !important;"><p id="" class="" >&nbsp;</p>If you have completed the Site Specific Induction for '.$client_company->company_name.' #'.$pj_details->project_id.'
...
//	page 30
	...
	if (file_exists('./docs/stored_docs/'.$file_name.'.pdf')) {
		...
		$old_file = $file_name.'_'.time();

		copy('./docs/stored_docs/'.$file_name.'.pdf', './docs/stored_docs/'.$old_file.'_old.pdf');
...
/*
		if($project_id == 43650){
			$final_total_quoted = $final_total_quoted - 0.01;
			$this->projects_m->update_project_total($project_id,$final_total_quoted);
			$this->re_allocate_invoice($project_id,$final_total_quoted);
		}
 */
...
*/
	...
	public function set_quote_review(){
...
*/
	...
	//	echo '<pre>';var_dump($_POST );echo '</pre>';
		...
		$work_ids = $_POST['quote_rev_ids'];
		$project_id_review = $_POST['project_id_review'];
		$quote_deadline_rvw = $_POST['quote_deadline_rvw'];
		$old_quote_date = $_POST['old_quote_date'];
		$work_id_list = implode(', ',$work_ids);

		if (strncmp($old_quote_date, $quote_deadline_rvw, 10) === 0) {
			$quote_deadline = 'Quote Deadline: '.$quote_deadline_rvw;
		}else{
			$quote_deadline = 'Old Quote Deadline: '.$old_quote_date.' to New Quote Deadline: '.$quote_deadline_rvw;
		}
		...
		$time = date("H:i:s");
		$type = "INSERT";
		$actions = $quote_deadline." <br /> Quote Review initialized for the following Work ID: ".$work_id_list;
		$this->user_model->insert_user_log($user_id,$date,$time,$actions,$project_id_review,$type);
		// user logs


		foreach ($work_ids as $key => $value) {
			$this->projects_m->set_project_quote_review($value);
		}

		$this->projects_m->update_quote_deadline($project_id_review,$quote_deadline_rvw);

		redirect('/projects/view/'.$project_id_review);
			...
			if($project_comments->num_rows > 0){
				...
				foreach ($project_comments->result_array() as $row){
					...
					if( $this->session->userdata('user_id') == 2 ){
						echo '<a href="'.base_url().'projects/rem_commnt/'.$row['project_comments_id'].'" target="_blank" class="btn btn-xs btn-danger pull-right err_commnt">Remove</a>';
					}
