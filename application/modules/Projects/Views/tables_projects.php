...
$get_table_status = $this->request->getPost('status');
...
$is_restricted = 0;
foreach ($proj_t->getResultArray() as $row){
			...
			if($is_restricted == 1){
				if($unaccepted_date == ""){
					if(strtotime($start_date) < strtotime($today)){
						...
						// auto feedback send
						$logs_for_jobdate = $this->projects_m->get_recent_log_jobdate($row['project_id']);
						
						if($logs_for_jobdate->getNumRows() > 0){
							$getResult_a = $logs_for_jobdate->getResult();

							$logs = array_shift($getResult_a);
							...
							if($days_diff > 1){
								...
								if($row['job_category'] != 'Maintenance' && $row['job_category'] != 'Minor Works' && $row['job_category'] != 'Design Works'){
									$this->projects->process_feedback_unaccepted($row['project_id']);
								}
			...
			if($row['unaccepted_date'] == ""){
				...
				if($is_restricted == 0){
					...
					// auto feedback send
						...
						if($logs_for_jobdate->getNumRows() > 0){
							$getResult_b = $logs_for_jobdate->getResult();

							$logs = array_shift($getResult_b);
									//echo '<pre>';var_dump($logs );echo '</pre>'; 
...
*/
			...
			<a href="'.site_url().'projects/update_project_details/'.$row['project_id'].'?status_rvwprj='.$status.'&pmr='.$row['project_manager_id'].'"><strong class="prj_id_rvw">'.$row['project_id'].''.'</strong> - '.$row['project_name'].'</a>';
...
*/
		...
		if ($status == 'quote'){
			echo '<tr class="'.$status.'"><td><a href="'.site_url().'projects/view/'.$row['project_id'].'?tab=works" >'.$row['project_id'].'</a></td><td>'.$row['project_name'].'</td><td>'.$company_name.'</td><td>'.$row['job_category'].'</td><td>'.($row['job_date'] == '' ? 'Unset' : $row['job_date']).'</td>';
		} else if ($status == 'invoiced') {
			echo '<tr class="'.$status.'"><td><a href="'.site_url().'projects/view/'.$row['project_id'].'?tab=invoice" >'.$row['project_id'].'</a></td><td>'.$row['project_name'].'</td><td>'.$company_name.'</td><td>'.$row['job_category'].'</td><td>'.($row['job_date'] == '' ? 'Unset' : $row['job_date']).'</td>';
		} else if ($status == 'paid') {
			echo '<tr class="'.$status.'"><td><a href="'.site_url().'projects/view/'.$row['project_id'].'?tab=project_details" >'.$row['project_id'].'</a></td><td>'.$row['project_name'].'</td><td>'.$company_name.'</td><td>'.$row['job_category'].'</td><td>'.($row['job_date'] == '' ? 'Unset' : $row['job_date']).'</td>';
		} else {
			echo '<tr class="'.$status.'"><td><a href="'.site_url().'projects/view/'.$row['project_id'].'" >'.$row['project_id'].'</a></td><td>'.$row['project_name'].'</td><td>'.$company_name.'</td><td>'.$row['job_category'].'</td><td>'.($row['job_date'] == '' ? 'Unset' : $row['job_date']).'</td>';
		}
		...
		if($this->session->get('user_id') == $row['project_manager_id'] ){
			echo '<td>PM</td>';
		}else if($this->session->get('user_id') == $row['project_admin_id'] ){
			echo '<td>PA</td>';
		}elseif($this->session->get('user_id') == $row['project_estiamator_id'] ){
			echo '<td>EST</td>';
		}else{
			echo '<td>ORD</td>';
		}
		...
		if ($warranty_date_format >= $date_today_format){
			...
			echo '<tr class="warranty"><td><a href="'.site_url().'projects/view/'.$row['project_id'].'" >'.$row['project_id'].'</a></td><td>'.$row['project_name'].'</td><td>'.$company_name.'</td><td>'.$row['job_category'].'</td><td>'.$row['warranty_date'].'</td>';
