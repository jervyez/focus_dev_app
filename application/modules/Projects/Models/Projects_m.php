...
class Projects_m{
	...
	public function get_prj_consmngr_leadhnd($project_id){
		$query = $this->db->query("SELECT `project`.`project_id`, `project_schedule`.`contruction_manager_id`  , `project_schedule`.`leading_hand_id`, `project`.`project_manager_id`,  `cm_user`.`user_id` AS `construction_mngr_id`,
			 `manual_entry_project_schedule`.`manual_entry_ps_id`, `manual_entry_project_schedule`.`lh_name`,`manual_entry_project_schedule`.`lh_contact` 
			FROM `project`  
			...
			LEFT JOIN `manual_entry_project_schedule` ON `manual_entry_project_schedule`.`project_schedule_id` = `project_schedule`.`project_schedule_id` 
			WHERE `project`.`project_id`  = '$project_id' 
			AND  `cm_user`.`user_role_id` = '11'  ");
	...
	public function insert_uploaded_file($file_name,$file_type,$project_id,$client_id,$date_upload,$user_id,$will_replace_existing){
		...
		if($will_replace_existing == 1):
			$is_authorized = 0;
			$storage_files_id = $this->db->insertID();
	...
	public function set_project_quote_review($works_id){
		$this->db->query(" UPDATE `work_contractors` SET `prj_quote_review` = '1' WHERE `work_contractors`.`works_id` = '$works_id' AND `work_contractors`.`ex_gst` IS NOT NULL ");
	}

	public function update_quote_deadline($project_id,$date){
		$this->db->query(" UPDATE `project` SET `quote_deadline_date` = '$date' WHERE `project`.`project_id` = '$project_id' ");
	}
...
INNER JOIN `company_details` ON `company_details`.`company_id` = `storage_files`.`client_id`
			...
			LEFT JOIN 	`users` ON `users`.`user_id` =  `storage_files`.`user_id`
...
ORDER BY  `storage_files`.`client_id`  DESC, `storage_doc_type`.`doc_type_name` ASC, UNIX_TIMESTAMP( STR_TO_DATE(`storage_files`.`date_upload`, '%d/%m/%Y') ) DESC ");
	...
	public function erase_comment($comment_id){
		$this->db->query(" DELETE FROM `project_comments` WHERE `project_comments`.`project_comments_id` = '$comment_id' ");
	}
	...
	public function insert_cost_total($project_id,$install_cost_total){
		...
		$cost_total_id = $this->db->insertID();
	...
	public function list_works_for_feedback($project_id){

		$query = $this->db->query(" SELECT  `project`.`project_id`, `works`.`works_id` FROM `project`
			LEFT JOIN `works` ON `works`.`project_id` =  `project`.`project_id`
			WHERE `project`.`receive_feedback` = '1'
			AND `project`.`is_active` = '1'
			AND `project`.`project_id` = '$project_id' 
			AND `works`.`receive_feedback` = 1
	...
	public function get_recent_log_jobdate($job_date){
		$query = $this->db->query("SELECT * FROM `user_log` WHERE `user_log`.`project_id` = '$job_date' AND `user_log`.`actions` LIKE '%Removed job date%' ORDER BY `user_log`.`user_log_id` DESC LIMIT 1");
		return $query;
	}
	...
	public function insert_new_project_sched_for_pr($project_id, $leading_hand_id){
		...
		$new_project_sched_id = $this->db->insertID();
	...
	public function auto_log_out_site_login(){
		$query = $this->db->query("SELECT *, DATE(login_datetime) as login_date FROM site_staff_site_login WHERE DATE(logout_datetime) = '0000-00-00 00:00:00'");
		foreach ($query->getResultArray() as $row){
	...
	public function enter_job_date($project_id){
		$this->db->query("UPDATE `project` SET `job_date_entered` = 1 WHERE `project_id` = '$project_id'");
	}

// Project Dashboard-----------------------------------
	public function fetch_project_works_sl_dashboard($project_id){
		$query = $this->db->query("SELECT a.*,
										if(contractor_type = 3, supplier_cat_name, if(work_con_sup_id = 82,other_work_desc,job_sub_cat)) as work_desc,
										`a`.`receive_feedback` AS `work_feedback` 
									FROM works a 
	...
	public function fetch_project_site_labour_logs($project_id){
		$query = $this->db->query("SELECT *
								FROM site_labour_computation 
								WHERE project_id = '$project_id'
								");
		return $query;	
	}
