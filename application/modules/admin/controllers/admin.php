...
class Admin extends MY_Controller{
	...
	function __construct(){
		...
		public function update_archive_details(){
			...
			redirect('/admin#archive_documents_settings');
		...
		public function update_allowed_user_rempend_comp(){
			$user_id = $this->input->post('allowed_user_id');
			$this->admin_m->update_allowed_users_remove_comp($user_id);
			redirect('/admin#onboarding_pending_company_removal');
		}
		...
		public function update_cc_emails(){
			...
			$static_emails = array_shift($q_static_emails->result());
			...
			$this->admin_m->update_cc_emails_static($set_cc_emails_cqr,$set_cc_emails_cpo,$set_cc_emails_cqr_reply,$set_cc_emails_induction);
		 	redirect('/admin#cc_emails_static_assignments');
		}

		public function update_closing_settings(){

			$alert_time_hr = $this->input->post('alert_time_hr');
			$alert_time_mins = $this->input->post('alert_time_mins');
			$alert_msg = $this->input->post('alert_msg');
			...
			redirect('/admin?ups=1#closing_settings_form');
		}
		...
		public function get_employee_supply_rem($focus_company){
			...
			return array_shift($q_get_employee_supply_reminder->result_array() );
		}
		...
		public function list_warehouse_location(){
			...
			return $q_get_warehouse_locations->result();		
		}

		public function list_assigned_warehouse(){
			$q_assigned_warehouse = $this->admin_m->get_assigned_warehouse();
			return $q_assigned_warehouse->result();
		}

		public function get_brands($form='table'){
	...
	//		echo $cat_pcc_dashboard;

			$this->admin_m->update_pcc_dashboard($cat_pcc_dashboard);
			redirect('/admin#update_cat_pcc');
		...
		//	$fetch_archive_emp = $this->admin_m->q_emp_archive();
			$archive_emp = $q_emp_archive->result();
			$emp_id = '';
			$archive_registry_id = '';
			$expiry = '';
			...
			if($q_emp_archive->num_rows > 0){
				foreach($archive_emp as $key => $archive_data){
					...
					echo '<span class="badge btn-primary  " style="margin: 2px 5px 0px;"><i  class="fa fa-user tooltip-enabled" data-original-title="'.$archive_data->user_first_name.' - '.$archive_data->expiry.'"  data-placement="right" style="margin: 0;"></i></span>';
				}
...
/*
		public function delete_arch_type($arch_id){
			var_dump($arch_id);
//		redirect('/admin#archive_documents_settings');
		}
*/
		...
		public function update_arc_doc_type(){
			...
			//array(2) { ["archive_name"]=> string(7) "Testing" ["archive_registry_types"]=> string(1) "1" }
		...
		//admin_defaulst
			...
			$data_c['all_focus_company'] = $all_focus_company->result();

			$all_email_banners = $this->admin_m->list_email_banners();
			$data_c['all_focus_company_banners'] = $all_email_banners->result();
		...
		//labour cost matrix
			...
			$data['select_leave_rates_salaried'] = $select_leave_rates_salaried->result();

			$select_leave_rates_wages = $this->admin_m->select_leave_rates('2');
			$data['select_leave_rates_wages'] = $select_leave_rates_wages->result();

			$select_leave_rates_manila = $this->admin_m->select_leave_rates('3');
			$data['select_leave_rates_manila'] = $select_leave_rates_manila->result();
	...
	public function get_seasons(){
		...
		foreach ($q_seasons->result() as $img_bg){
			...
			public function upload_email_banner(){
				...
				$config['file_name']  = 'email_signature_'.$time;

				$this->load->library('upload', $config);
				$this->upload->initialize($config);

				$this->upload->do_upload('signature_banner');

				$banner_file_name_arr = $this->upload->data('file_name');  
				$banner_focus_id = $this->input->post('banner_focus_comp', true);

				$this->admin_m->update_email_banner($banner_focus_id,$banner_file_name_arr['file_name']);
				redirect('/admin');
			}

			public function update_cqr_template(){
				$email_msg_cqr = $this->input->post('email_msg_cqr');
...
//		redirect('/admin');
	...
	//	}
			...
			//	redirect( $redirect_back );
				...
				redirect('/users/account/'.$pa_user_id);
...
//		$admin_defaults_id = $this->admin_m->update_insert_quote_deadline($days_quote_deadline);
			...
			public function salaried_rates(){
				...
				$this->form_validation->set_rules('salaried_total_annual_leave', 'Salaried Total Annual Leave','trim|required|xss_clean');
				$this->form_validation->set_rules('salaried_total_personal_leave', 'Salaried Total Personal Leave','trim|required|xss_clean');

				if($this->form_validation->run() === false){
					$passArr['user_settings_error'] = validation_errors();
					$this->index($passArr);
				}else{
					$salaried_total_annual_leave = $this->input->post('salaried_total_annual_leave', true);
					$salaried_total_personal_leave = $this->input->post('salaried_total_personal_leave', true);
					...
					if ($q_fetch_users_by_leave_rate->num_rows() > 0){
					   foreach ($q_fetch_users_by_leave_rate->result() as $row){
					...
					$this->session->set_flashdata('update_user_settings_leave_rates', $update_success);
					redirect('/admin');
				}
			...
			public function wages_rates(){
				...
				$this->form_validation->set_rules('wages_total_annual_leave', 'Wages Total Annual Leave','trim|required|xss_clean');
				$this->form_validation->set_rules('wages_total_personal_leave', 'Wages Total Personal Leave','trim|required|xss_clean');
				$this->form_validation->set_rules('wages_total_rdo', 'Wages Total RDO Leave','trim|xss_clean');

				if($this->form_validation->run() === false){
					$passArr['user_settings_error'] = validation_errors();
					$this->index($passArr);
				}else{
					$wages_total_annual_leave = $this->input->post('wages_total_annual_leave', true);
					$wages_total_personal_leave = $this->input->post('wages_total_personal_leave', true);
					$wages_total_rdo = $this->input->post('wages_total_rdo', true);
			...
			public function manila_rates(){
				...
				$this->form_validation->set_rules('manila_total_vacation_leave', 'Total Vacation Leave','trim|required|xss_clean');
				$this->form_validation->set_rules('manila_total_sick_leave', 'Total Sick Leave','trim|required|xss_clean');

				if($this->form_validation->run() === false){
					$passArr['user_settings_error'] = validation_errors();
					$this->index($passArr);
				}else{
					$manila_total_vacation_leave = $this->input->post('manila_total_vacation_leave', true);
					$manila_total_sick_leave = $this->input->post('manila_total_sick_leave', true);
...
*/
	...
	public function default_email_message_onboarding_bank(){
		$this->clear_apost();
		$sender_name_onboarding_bank = $this->input->post('sender_name_onboarding_bank', true);
		$sender_email_onboarding_bank = $this->input->post('sender_email_onboarding_bank', true);
		$subject_onboarding_bank = $this->input->post('subject_onboarding_bank', true);
		$email_msg_onboarding_bank = $this->input->post('email_msg_onboarding_bank', true);
		$bcc_email_onboarding_bank = $this->input->post('bcc_email_onboarding_bank', true);
		$user_id = $this->input->post('user_assigned_onboarding_bank', true);
...
// induction end =========================
	...
	public function pdf_do_upload() {
		...
		$upload_path_pdf = $_POST['upload_path_pdf'];
	   	...
	   	$config['upload_path'] = './docs/'.$upload_path_pdf;
	   	$config['allowed_types'] = 'pdf';
	   	$config['max_size'] = 0;

	   	if ($state == 'wa'){
	   		$config['file_name'] = 'bank_details_form_wa';
	   	}

	   	if ($state == 'nsw'){
	   		$config['file_name'] = 'bank_details_form_nsw';
	   	}
	...
	public function save_work_filter_default(){
		$data = json_decode(file_get_contents("php://input"), true);
		
		$works_restriction_categories = "";
		
		foreach ($data['works_restriction_categories'] as $value) {
			if($works_restriction_categories == ""){
				$works_restriction_categories = $value;
			}else{
				$works_restriction_categories = $works_restriction_categories .",". $value;
			}
	  		
		}

		$this->admin_m->update_works_restriction_categories($works_restriction_categories);
