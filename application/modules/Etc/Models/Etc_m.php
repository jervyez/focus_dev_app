<?php
namespace App\Modules\Etc\Models;
...
class Etc_m{
    ...
    public function get_work_contractor_details($work_contractor_id){
        $query = $this->db->query(" SELECT `works`.`work_reply_date`,`works`.`work_cpo_date`,`project`.`focus_company_id`,`work_contractors`.`works_contrator_id`, `work_contractors`.`works_id` ,`works`.`work_replyby_time`,`work_contractors`.`ex_gst`,`project`.`job_type`, `work_contractors`.`ex_gst`,
            ...
            UNIX_TIMESTAMP( STR_TO_DATE(`works`.`work_reply_date`, '%d/%m/%Y') ) AS `unix_reply_date`, `works`.`price`,`works`.`receive_feedback` AS `set_wrk_fb`  ,`work_contractors`.`set_send_feedback`,`project`.`receive_feedback` AS `set_prj_fb` ,`work_contractors`.`contractor_reply_conditions`,`work_contractors`.`inc_gst`, `project`.`is_pending_client`,`works`.`other_work_desc`,`project`.`project_admin_id`,
    ...
    public function list_group_contractors_per_selected($works_contrator_id){
        $query = $this->db->query("SELECT  `work_contractors`.`works_contrator_id`,`work_contractors`.`ex_gst` FROM `work_contractors` 
            ...
            AND `work_contractors`.`set_send_feedback` = '1'
            AND  `work_contractors`.`feedback_date` IS NULL ORDER BY `work_contractors`.`is_selected` DESC");
        return $query;
    }

    public function list_group_contractors_per_workid($work_id){
        $query = $this->db->query("SELECT  `work_contractors`.`works_contrator_id`,`work_contractors`.`ex_gst` FROM `work_contractors` 
            WHERE`work_contractors`.`works_id` = '$work_id'
            AND `work_contractors`.`set_send_feedback` = '1'
            AND  `work_contractors`.`feedback_date` IS NULL ORDER BY `work_contractors`.`is_selected` DESC");
        return $query;
    }
    ...
    public function get_best_quoted_price_per_workid($work_id){
        $query = $this->db->query("SELECT `work_contractors`.`works_contrator_id`,`work_contractors`.`ex_gst` FROM `work_contractors` 
            WHERE`work_contractors`.`works_id` = '$work_id'
            AND `work_contractors`.`ex_gst` > 0 ORDER BY `work_contractors`.`ex_gst` ASC LIMIT 1");
        return $query;
    ...
    public function get_email_feedbacks($unaccepted=''){

        if($unaccepted == 1 && isset($unaccepted)){
            $query = $this->db->query(" SELECT  `unaccepted_selected_email` AS `selected_contractor_email`, `unaccepted_unselected_email` AS `unsuccessful_contractor_email` FROM `static_defaults` ");
        }else{
